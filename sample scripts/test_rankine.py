...
ran = typhoon_model.Rankine(tm)
typhoon = ran.optimize(submethod = "SGP02")         #apply submethod if no data points present

#1D Profile
import matplotlib.pyplot as plt

rs = np.arange(0, 1000.0, 1.0)                   #creates an array of r distances
typhoon = ran.profiler(rs)                   #calculates gradient wind at r distances away
...
for index, entry in typhoon.iterrows():
    ...
    plt.title(f"t={index}hr, X={entry.X:0.2f}")
    plt.grid()
    plt.xlabel("radius (km)")
    plt.ylabel("gradient wind speed (m/s)")
    plt.ylim([0, 90])
    plt.xlim([0, 1000])
    fname = f"Rankine_Profile\\t={index}hr.png"
    plt.savefig(fname)
    plt.show()
    plt.close()
    
typhoon.to_csv(f"Rankine_Profile\\00 - typhoon.csv")    
...
ran.field_maker(grid, north=True)                         #north is True if northern hemisphere
...
anim.save('Haiyan_Rankine.mp4', fps=5, extra_args=['-vcodec', 'libx264'])
