...
for index, entry in typhoon.iterrows():
    ...
    plt.title(f"t={index}hr, B={entry.B:0.2f}")
    ...
    plt.ylim([0, 90])
    plt.xlim([0, 1000])
    fname = f"Holland_Profile\\t={index}hr.png"
    plt.savefig(fname)
    plt.show()
    plt.close()

typhoon.to_csv(f"Holland_Profile\\00 - typhoon.csv")    
...
hol.field_maker(grid, north=True)                         #north is True if northern hemisphere
